...
const SET_DURATION = 45; // Default "Work" time guideline (seconds)
const REST_DURATION = 30; // Rest time (seconds)
...
const CircularTimer: React.FC<{ 
  ...
  const color = isRest ? '#10b981' : '#00d2ff'; // Green for Rest, Blue for Work
...
const ActiveWorkoutPlayer: React.FC<ActiveWorkoutPlayerProps> = ({ plan, onComplete, onFail }) => {
    ...
    if (!isPaused && timeLeft > 0) {
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          ...
          // Halfway Alert
          if (phase === 'WORK' && next === Math.floor(SET_DURATION / 2)) {
             SpeechService.announceHalfway();
          }
          // Countdown Ticks
          if (next <= 3 && next > 0) {
  ...
  const handleTimerComplete = () => {
    if (phase === 'WORK') {
      ...
      // Log simple completion (1 rep effectively for tracking set completion)
  ...
  const handleExerciseComplete = () => {
    if (currentIdx < totalExercises - 1) {
      ...
      // Transition to next exercise (Rest first? Or direct? Let's do a short transition rest)
      setPhase('REST');
      setTimeLeft(REST_DURATION); // Rest before next exercise
  ...
  const skipTimer = () => {
    setTimeLeft(0); // Triggers useEffect(timeLeft === 0)
  };
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col font-sans text-white">
        ...
        <div className="flex justify-between items-center p-6 bg-black border-b border-gray-900 z-20">
            ...
            <button 
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-900 text-gray-500 hover:bg-red-900/20 hover:text-red-500 transition-colors"
            >
               <X size={20} />
            </button>
        </div>
        ...
        <div className="flex-1 relative flex flex-col">
            ...
            <div className="flex-1 flex flex-col justify-center relative bg-black/50 border-y border-system-border overflow-hidden group">
               ...
               <div className={`w-full h-full relative flex items-center justify-center transition-opacity duration-500 ${phase === 'WORK' ? 'opacity-100' : 'opacity-50 blur-sm'}`}>
                   ...
                   {/* Corner Brackets */}
                   <div className="absolute top-4 left-4 w-8 h-8 border-t-2 border-l-2 border-system-neon/50" />
                   <div className="absolute bottom-4 right-4 w-8 h-8 border-b-2 border-r-2 border-system-neon/50" />
               </div>
               ...
               {phase === 'REST' && (
                  <motion.div 
                    ...
                    className="absolute inset-0 flex flex-col items-center justify-center pb-8 z-30 pointer-events-none"
                  >
                     <div className="bg-black/90 backdrop-blur-md border border-system-success/50 px-10 py-8 rounded-xl text-center shadow-[0_0_50px_rgba(16,185,129,0.3)]">
                        ...
                        <p className="text-xs text-gray-400 font-mono">
                           UP NEXT: <span className="text-system-neon font-bold">{plan.exercises[currentIdx]?.name || 'FINISH'}</span>
                        </p>
                     </div>
                  </motion.div>
               )}
            ...
            <div className="bg-[#050505] border-t border-gray-900 p-8 pb-12 flex flex-col items-center relative z-20 shadow-[0_-20px_50px_rgba(0,0,0,0.8)]">
               ...
               <div className="mb-6 flex gap-1">
                  {Array.from({ length: exercise.sets }).map((_, i) => (
                     <div 
                       key={i} 
                       className={`w-8 h-1 rounded-full transition-colors ${i < currentSet - (phase === 'REST' && currentSet > 1 ? 0 : 1) ? 'bg-system-neon' : i === currentSet - 1 ? 'bg-white animate-pulse' : 'bg-gray-800'}`}
                     />
                  ))}
               </div>
               ...
               <div className="flex items-center gap-8 w-full max-w-md justify-between">
                  ...
                  <button 
                    onClick={skipTimer}
                    className="flex-1 py-4 rounded-xl bg-gray-900 border border-gray-800 text-gray-400 font-bold font-mono text-xs hover:bg-gray-800 hover:text-white transition-all active:scale-95"
                  >
                     {phase === 'WORK' ? 'COMPLETE EARLY' : 'SKIP REST'}
                  </button>
                  ...
                  <div className="-mt-16 bg-black rounded-full p-2 border-4 border-[#050505]">
                     <CircularTimer 
                        progress={timeLeft} 
                        total={phase === 'WORK' ? SET_DURATION : REST_DURATION} 
                        isRest={phase === 'REST'}
                        isPaused={isPaused}
           ...
           {showQuitConfirm && (
              <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-md flex items-center justify-center p-6">
                 <motion.div 
                   ...
                   className="bg-[#0a0a0a] border border-red-900/50 w-full max-w-sm rounded-xl p-6 text-center shadow-[0_0_50px_rgba(220,38,38,0.2)]"
                 >
                    ...
                    <p className="text-xs text-red-400 font-mono mb-6 leading-relaxed">
                       WARNING: Leaving the instance early will result in a penalty. XP will be deducted and your streak may be broken.
                    </p>
                    <div className="flex gap-3">
                       <button 
                         onClick={() => setShowQuitConfirm(false)}
                         className="flex-1 py-3 rounded bg-gray-800 text-white font-bold text-xs hover:bg-gray-700"
                       >
                          RESUME
                       </button>
                       <button 
                         onClick={confirmQuit}
                         className="flex-1 py-3 rounded bg-red-900/20 border border-red-900 text-red-500 font-bold text-xs hover:bg-red-900 hover:text-white transition-colors"
                       >
                          ACCEPT PENALTY
                       </button>
                    </div>
