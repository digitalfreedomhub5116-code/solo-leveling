...
import { Activity, Target, Dumbbell, Flame, CheckCircle, TrendingUp, ChevronLeft, ChevronRight, Ruler, Fingerprint, Crown, Eye, ChevronDown, ChevronUp, ShieldCheck, Trophy, Clock, Zap, Calculator, Apple, Camera, Upload, Trash2, Maximize2, Search, Plus, Utensils, PieChart, ScanLine, X, AlertTriangle, RefreshCw, Droplets, Moon, Calendar } from 'lucide-react';
...
const ResultsView: React.FC<{ profile: Partial<HealthProfile>; onConfirm: () => void }> = ({ profile, onConfirm }) => {
    ...
    const rate = profile.goal === 'BUILD_MUSCLE' ? 0.25 : 0.5; // kg per week
    const weeks = weightDiff === 0 ? 0 : Math.max(4, Math.ceil(weightDiff / rate));
    const waterIntake = Math.round((profile.weight || 70) * 0.033 * 10) / 10; // Liters
