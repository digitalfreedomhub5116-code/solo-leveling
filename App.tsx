...
// Nav Order for Swipe Logic
const TABS_ORDER: Tab[] = ['DASHBOARD', 'HEALTH', 'QUESTS', 'ARMORY', 'ALLIANCE'];
...
const App: React.FC = () => {
  ...
  const handleGlobalSwipe = (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
      if (isDungeonMode || showCheatWarning || isPenalty || activeTab === 'CASTLE' || onboardingPhase !== 'APP') return;

      // Ignore if swipe originates from a "no-swipe" zone (e.g. inner horizontal scroll)
      let target = event.target as HTMLElement;
      while(target && target !== document.body) {
          if (target.getAttribute && target.getAttribute('data-no-swipe') === 'true') return;
      ...
      if (Math.abs(info.velocity.x) < VELOCITY_THRESHOLD) return;
  ...
  const handleTutorialNext = () => {
      ...
      if (nextStep === 2) handleTabChange('QUESTS'); 
      if (nextStep === 8) handleTabChange('HEALTH'); 
  ...
  return (
    <>
      ...
      <Layout 
        ...
        onGoldClick={!isDungeonMode && !showCheatWarning ? () => handleTabChange('ARMORY') : undefined}
      >
        <motion.div
            className="w-full h-full min-h-[80vh] touch-pan-y"
            onPanEnd={(e, info) => handleGlobalSwipe(e, info)}
        >
                        ...
                        {isContentLoading ? (
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    {/* Using new glass-panel for sections */}
                                    <div id="tut-stats" className="lg:col-span-2 glass-panel rounded-xl overflow-hidden p-0 relative group">
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-white/5 text-[10px] text-gray-400 font-bold uppercase tracking-widest border-b border-white/10">
                                            ...
                                            <tbody className="font-mono text-xs">
                                                {(Object.keys(STAT_CONFIG) as Array<keyof CoreStats>).map((key) => {
                                                    ...
                                                    return (
                                                        <tr key={key as string} className="border-b border-white/5 hover:bg-white/10 transition-colors last:border-0">
                                                            <td className="p-3 pl-6 flex items-center gap-3">
                                                                <div className={`p-2 rounded bg-white/5 border border-white/10 ${config.color}`}>
                                                                    <config.icon size={14} />
                                                                </div>
                                                            ...
                                                            <td className="p-3 pr-6">
                                                                <div className="h-1.5 w-full bg-gray-900/50 rounded-full overflow-hidden border border-white/10">
                                                                    <motion.div 
                                                                        initial={{ width: 0 }}
                                    ...
                                    <div className="flex flex-col justify-start gap-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <button 
                                                ...
                                                className="flex items-center justify-center gap-2 text-[10px] text-gray-400 hover:text-white transition-colors font-mono tracking-widest group border border-white/10 hover:border-white/30 px-3 py-4 rounded bg-white/5 backdrop-blur-sm"
                                            >
                                            ...
                                            <button 
                                                onClick={logout}
                                                className="flex items-center justify-center gap-2 text-[10px] text-red-400 hover:text-red-300 transition-colors font-mono tracking-widest group border border-red-500/20 hover:border-red-500/40 px-3 py-4 rounded bg-red-950/10 backdrop-blur-sm"
                                            >
                                                <LogOut size={14} />
                                                LOGOUT
                                            </button>
                                        ...
                                        <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between text-[10px] font-mono text-gray-400 backdrop-blur-sm">
