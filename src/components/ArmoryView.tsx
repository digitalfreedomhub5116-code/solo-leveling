...
interface ArmoryViewProps {
  // Using generic props as we might hook this up to global state later
  gold: number;
...
const ArmoryView: React.FC<ArmoryViewProps> = ({ 
  ...
  // Find index of equipped outfit initially if possible, else 0
  ...
  // Mock equipped shadows for visualization (In a real app, pass via props)
  // Slot 0: Empty, Slot 1: Igris, Slot 2: Empty
  ...
  return (
    <div className="min-h-screen text-gray-100 font-sans pb-24 overflow-hidden flex flex-col items-center bg-transparent">
      
      {/* 1. OUTFIT CAROUSEL */}
      <div className="relative w-full max-w-xl h-[45vh] flex items-center justify-center border-b border-white/10 overflow-hidden bg-black/20 backdrop-blur-sm rounded-b-3xl">
        
        {/* Background Gradient for Depth - Transparent base */}
        ...
        <AnimatePresence mode='wait'>
          ...
          >
            {/* Image Container */}
            <div className="w-48 h-64 md:w-56 md:h-72 bg-black/40 rounded-2xl mb-4 border border-white/10 shadow-2xl relative overflow-hidden group">
               {/* Scan Effect */}
               <div className="absolute top-0 w-full h-1 bg-system-neon/50 shadow-[0_0_15px_#00d2ff] animate-[scan_2s_ease-in-out_infinite] z-20 pointer-events-none" />
            ...
            <div className={`inline-block px-3 py-0.5 border border-current rounded mt-2 text-[10px] font-bold tracking-widest ${tierInfo.color} bg-black/50 backdrop-blur-md`}>
            ...
            <div className="mt-4">
                {isUnlocked ? (
                    <button 
                        ...
                        disabled={isEquipped}
                        className={`px-6 py-2 rounded font-black text-xs uppercase tracking-widest transition-all ${isEquipped ? 'bg-gray-800 text-gray-500 cursor-default' : 'bg-white text-black hover:bg-system-neon hover:scale-105 shadow-[0_0_15px_rgba(255,255,255,0.3)]'}`}
                    >
                    ...
                    <button 
                        ...
                        className="px-6 py-2 bg-yellow-600 text-black font-black text-xs uppercase tracking-widest rounded hover:bg-yellow-500 transition-all shadow-[0_0_15px_rgba(234,179,8,0.4)]"
                    >
      ...
      {/* 2. STATS SECTION */}
      <div className="w-full max-w-xl p-6 space-y-2 glass-panel mt-4 rounded-xl">
        <div className="flex items-center gap-2 mb-4 border-b border-gray-700 pb-2">
            <Zap size={16} className="text-system-neon" />
      ...
      {/* 3. SHADOW ARMY */}
      <div className="w-full max-w-xl p-6 mt-4 glass-panel rounded-xl">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-sm font-black text-purple-500 flex items-center gap-2 uppercase tracking-widest">
            <Ghost size={16} /> Shadow Army
          </h3>
          <span className="text-[10px] text-gray-400 font-mono font-bold tracking-wider">3 SLOTS AVAILABLE</span>
        </div>
            ...
            <div key={index} className="aspect-square relative group">
              {shadow ? (
                ...
                <motion.div 
                  ...
                  className="w-full h-full rounded-xl bg-gray-900/80 border border-purple-500 overflow-hidden relative shadow-[0_0_15px_rgba(168,85,247,0.3)] group cursor-pointer backdrop-blur-sm"
                >
                  {/* Image Placeholder */}
                  <div className="absolute inset-0 bg-purple-900/20 flex items-center justify-center text-2xl font-black text-purple-700 uppercase">
                    {shadow.name[0]}
                  </div>
                ...
                <motion.button 
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="w-full h-full rounded-xl border-2 border-dashed border-gray-800 flex flex-col items-center justify-center gap-2 hover:border-system-neon hover:bg-system-neon/5 transition-colors group bg-black/20"
                >
                  <Plus className="text-gray-600 group-hover:text-system-neon transition-colors" />
                  <span className="text-[8px] text-gray-600 font-bold uppercase tracking-wider group-hover:text-system-neon">Equip</span>
                </motion.button>
              )}
            </div>
          ))}
